...
// 2. Obtener una orden específica por su ID
...
// 3. Crear una nueva orden de trabajo (Ingreso)
export const crearOrden = async (req, res) => {
    try {
        ...
        // Buscar el ultimo codigo de orden con el prefijo del dia
        ...
        if (ultimo.length > 0) {
            ...
            // Extraer el numero: ORD-260710-0001 → "0001"
...
// 4. Actualizar el estado de la orden (y el diagnóstico)
export const actualizarEstadoOrden = async (req, res) => {
    try {
        ...
        console.log('Backend - Actualizando orden ID:', id);
        console.log('Backend - Nuevo estado:', estado_reparacion);
        console.log('Backend - Usuario ID:', userId);
        console.log('Backend - Usuario Rol:', userRol);
        ...
        if (userRol !== 'ADMIN' && orden[0].id_tecnico !== userId) {
            return res.status(403).json({ 
                error: 'No tienes permisos para cambiar el estado de esta orden. Solo el técnico asignado puede hacerlo.' 
            });
        }
        ...
        // Si el estado es ENTREGADO, verificar que no haya saldo pendiente
        if (estado_reparacion === 'Entregado') {
            ...
            if (saldoPendiente > 0) {
                return res.status(400).json({
                    error: 'No se puede entregar la orden. El cliente tiene saldo pendiente: $' + saldoPendiente.toFixed(2),
        ...
        const [ordenActualizada] = await conmysql.query(
...
export const actualizarOrden = async (req, res) => {
    try {
        ...
        if (userRol !== 'ADMIN' && orden[0].id_tecnico !== userId) {
            return res.status(403).json({ 
                error: 'No tienes permisos para editar esta orden. Solo el técnico asignado puede modificarla.' 
            });
        }
...
export const getRepuestosByOrden = async (req, res) => {
    try {
        ...
        console.error('Error al obtener repuestos de la orden:', error);
...
export const agregarRepuestoAOrden = async (req, res) => {
    ...
    try {
        ...
        console.log('Backend - Agregando repuesto a orden:', { id_orden, id_repuesto, cantidad_usada, precio_aplicado });
        ...
        // Insertar en detalles_reparacion
...
export const eliminarRepuestoDeOrden = async (req, res) => {
    ...
    try {
        ...
        // Obtener el detalle para saber qué repuesto y cantidad
        ...
        await conexion.query(
            `DELETE FROM detalles_reparacion WHERE id_detalle = ?`,
            [id]
        );
...
//  ACTUALIZAR CANTIDAD DE UN REPUESTO EN LA ORDEN
export const actualizarCantidadRepuesto = async (req, res) => {
    ...
    try {
        ...
        // Actualizar cantidad
        await conexion.query(
            `UPDATE detalles_reparacion SET cantidad_usada = ? WHERE id_detalle = ?`,
            [cantidad_usada, id]
        );
...
// ELIMINAR SERVICIO DE UNA ORDEN
export const eliminarServicioDeOrden = async (req, res) => {
    ...
    try {
        ...
        // Eliminar el detalle
        await conexion.query(
            `DELETE FROM detalles_servicios WHERE id_detalle_servicio = ?`,
            [id]
        );
...
export const getRepuestosMasUsados = async (req, res) => {
    try {
        ...
        console.error('Error al obtener repuestos más usados:', error);
        res.status(500).json({ error: 'Error al obtener estadísticas' });
    }
};

// Obtener últimas órdenes (5)
...
export const getOrdenesPorEstadoFiltrado = async (req, res) => {
    try {
        ...
        // Si no hay estado, devolver error
        if (!estado) {
...
// Verificar garantía de una orden específica
...
// Obtener garantías por vencer (próximos 7 días)
...
// Resumen de garantías (contadores)
export const getResumenGarantias = async (req, res) => {
    try {
        ...
        console.error('Error al obtener resumen de garantías:', error);
